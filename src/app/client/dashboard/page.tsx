...
import MobileClientDashboard from "@/components/client/MobileClientDashboard";
...
export default function ClientDashboardPage() {
  ...
  return (
    <>
      <MobileClientDashboard name={name || "موکل"} items={items} />

      <div className="mx-auto hidden max-w-5xl px-4 py-10 lg:block">
        <motion.section
        ...
        >
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            <Link href="/listings" className="rounded-full bg-emerald-600 px-4 py-2.5 text-sm text-white">
              آرشیو کامل
            </Link>
            <Link href="/contact" className="rounded-full bg-slate-100 px-4 py-2.5 text-sm text-slate-700">
              مشاوره اختصاصی
            </Link>
          </div>
        </motion.section>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          {picks.map((item) => (
            <Link
              ...
              className="overflow-hidden rounded-[1.5rem] bg-white shadow-sm ring-1 ring-slate-200"
            >
